...
def convert_videos_to_tfrecord2(source_path, destination_path,
  ...
  for i, batch in enumerate(filenames_split):
    data,names_to_append = convert_video_to_numpy(filenames=batch, width=width, height=height,
                                  n_frames_per_video=n_frames_per_video,
                                  n_channels=n_channels,
                                  dense_optical_flow=dense_optical_flow)
    print(type(names_to_append))
    ...
    save_numpy_to_tfrecords(data, names_to_append,destination_path, 'batch_',
                            n_videos_in_record, i + 1, total_batch_number,
                            color_depth=color_depth)


def save_numpy_to_tfrecords(data, names_to_append,destination_path, name, fragmentSize,
  ...
  for video_count in range((num_videos)):
    ...
    for image_count in range(num_images):
      ...
      feature['globs'] = _int64_feature(1)
...
def convert_video_to_numpy(filenames, n_frames_per_video, width, height,
  ...
  convert_videos_to_tfrecord2("/static/video.mp4", "/inputFiles/TFXFolder/bookinputTfT", 1, 5, "*.mp4")
